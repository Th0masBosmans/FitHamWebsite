...
/** Zelfde invulveld-stijl voor alle inlogschermen van het beheer. */
...
/** De tweede, lichtere knop, bv. "Annuleren". */
...
type AdminAuthLayoutProps = {
  /** Titel op de kaart. */
  title: string;
  /** Regeltje onder de titel. */
  subtitle: string;
  /** Linkje linksboven om weg te gaan. */
  backLink: { href: string; label: string };
  /** De inhoud van de kaart, meestal het formulier. */
  children: ReactNode;
}
...
export function AdminAuthLayout({ title, subtitle, backLink, children }: AdminAuthLayoutProps) {
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-[var(--color-primary-brand-darker)] via-[var(--color-primary-brand)] to-[var(--color-primary-brand-dark)] p-6">
      {/* Achtergrond: wazige clubkleuren + vaag patroon uit public/assets */}
