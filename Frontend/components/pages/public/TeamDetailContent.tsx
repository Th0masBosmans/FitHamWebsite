...
} from "@/repository/volleyRepository";
import { TeamStaffList } from "@/components/sections/teams/TeamStaffList";
import { TeamRoster } from "@/components/sections/teams/TeamRoster";
import { TrainingSchedule } from "@/components/sections/teams/TrainingSchedule";
import { NextMatchCard } from "@/components/sections/teams/NextMatchCard";
import { StandingsTable } from "@/components/sections/teams/StandingsTable";
...
/**
 * De pagina van één team. Van boven naar onder: de teamfoto met de naam erover,
 * een tekstje, de staf, de spelerslijst, de trainingsuren naast de
 * eerstvolgende wedstrijd, en onderaan de rangschikking.
 *
 * De gegevens komen uit twee bronnen: het team zelf uit onze eigen database, en
 * de wedstrijd en de rangschikking rechtstreeks van VolleyAdmin. Dat laatste
 * gebeurt alleen als de beheerder bij het team een reeks ingevuld heeft.
 */
export function TeamDetailContent({ teamId }: TeamDetailContentProps) {
  ...
  // Eerst het team zelf ophalen uit de database.
  useEffect(() => {
    let active = true;
    setLoadingTeam(true);
    teamRepository
  ...
  return (
    ...
    >
      {/* Wazige gekleurde vlekken op de achtergrond */}
      ...
      <div className="relative mx-auto max-w-md px-6 py-8 lg:max-w-6xl">
        {/* Terugknop naar het teamoverzicht */}
        <Link
          href="/teams"
        ...
        {/* Teamfoto over de volle breedte, met de teamnaam linksonder erover */}
        ...
        {/* Tekstje over het team (optioneel) */}
        {team.description && (
