...
export function Footer() {
  ...
  const getVariantsMobile = (position: string) => {
    // Mobiel: 1 groot logo in het midden, 2 kleine ernaast
    switch (position) {
      case "center":
  ...
  const getVariantsDesktop = (position: string) => {
    // Op desktop: 3 grote logo's in het midden, 2 kleinere ernaast
    switch (position) {
      case "center":
        return { x: "0%", scale: 1, opacity: 1, zIndex: 10 };
      case "prev":
    ...
    <footer className="mt-8 bg-(--color-primary-brand)/95 backdrop-blur-md border-t-4 border-(--color-accent)">
      <div className="max-w-md lg:max-w-6xl mx-auto px-6 py-6">
        ...
        <div className="mb-8">
          ...
          {/* Carrousel die vanzelf doorschuift; klikken brengt je naar de sponsorpagina */}
          <Link
            href="/sponsors"
          ...
          {/* Bolletjes om zelf een sponsor te kiezen */}
        ...
        {/* Adres van de sporthal (opent Google Maps) + het slotje naar het beheer */}
